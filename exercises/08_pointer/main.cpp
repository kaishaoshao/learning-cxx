...
bool is_fibonacci(int *ptr, int len, int stride) {
    ...
    for (int i = 0; i < len - 2; ++i) {
        int nextIndex = i + stride;
        int nextNextIndex = nextIndex + stride;
        if (nextIndex >= len || nextNextIndex >= len) {
            return false; // 确保不会越界
        }
        if (ptr[i] + ptr[nextIndex] != ptr[nextNextIndex]) {
            return false; // 检查斐波那契数列的性质
        }
    }
    return true;
}
